...
class RedshiftWorkloadAdapter(WorkloadAdapter):
    ...
    def render_ddl(self, proposals: list[Proposal]) -> str:
        ...
        **The header is far louder than `PostgresWorkloadAdapter.render_ddl`'s, and that is
        the single most important thing about this method.** Every ADV101 (`ALTER
        SORTKEY`), ADV102 (`ALTER DISTKEY`) and ADV103 (`ALTER DISTSTYLE ALL`) statement
        this adapter can emit rewrites the entire table: Redshift copies every row, holds a
        lock on the table for the whole rewrite, and needs roughly the table's own size
        again in free disk space while it runs — on a large table that is hours, not
        seconds. Postgres's header can recommend `CONCURRENTLY` for exactly this kind of
        risk; there is no such escape on Redshift, so the header says plainly that these
        three should be scheduled for a maintenance window rather than run ad hoc, and
        names `ADV104` (`VACUUM`/`ANALYZE`) as the one statement in this file that is *not*
        a rewrite. Each individual proposal's own `note` (see `propose_sortkey`,
        `propose_distkey`, `propose_diststyle_all`, `propose_maintenance`) repeats the
        specific cost right above its own statement — the header exists for the reader who
        only skims the top of a long file before scrolling.

        **ADV105 is attributed here, in the file itself, not only in the report.** Its DDL
        is Amazon Redshift Advisor's own recommendation, verbatim — never sqlquality's
        inference — so every ADV105 statement's header line below carries an explicit
        `(Amazon Redshift Advisor — not sqlquality)` marker in addition to its own `note`
        (already worded to say the same thing), so a reader skimming only the one-line
        headers, not every note, still cannot mistake an Advisor statement for one this
        tool generated.
        ...
        header = [
            ...
            "--",
            "-- DANGER: ADV101 (ALTER SORTKEY), ADV102 (ALTER DISTKEY) and ADV103 (ALTER",
            "-- DISTSTYLE ALL) below EACH REWRITE THE ENTIRE TABLE. Redshift copies every",
            "-- row, holds a lock on the table for the whole rewrite, and needs roughly",
            "-- the table's own size again in free disk space while it runs — on a large",
            "-- table that is HOURS, not seconds. There is NO CONCURRENTLY EQUIVALENT on",
            "-- Redshift: unlike a Postgres index, none of these three can be built or",
            "-- applied alongside normal traffic. SCHEDULE each one for a maintenance",
            "-- window; do not run it ad hoc.",
            "--",
            "-- ADV104 (VACUUM / ANALYZE) is the ONLY statement below that is not a full",
            "-- table rewrite: no exclusive lock for its duration, no full-table copy —",
            "-- safe to run without scheduling a maintenance window.",
            "--",
            "-- ADV105 statements are Amazon Redshift Advisor's OWN recommendations, not",
            "-- sqlquality's — each one is marked below and its own note repeats this.",
            "",
        ]
        body: list[str] = []
        for proposal in proposals:
            ...
            if _has_line_break(proposal.ddl) and not _is_fully_commented(proposal.ddl):
                ...
                # leave a bare-looking line in a file meant to be safe to skim. Comment the
                # whole thing out instead of guessing at a truncation.
            ...
            # ADV105's DDL is Redshift Advisor's, never sqlquality's own — see this
            # method's own docstring. Marked in the one-line header every statement gets,
            # not only in `note` below, so a reader skimming headers alone still sees it.
            attribution = (
                " (Amazon Redshift Advisor — not sqlquality)" if proposal.code == "ADV105" else ""
            )
            body.append(
                f"-- {proposal.code} [{proposal.confidence.value}{share_text}]{attribution}"
            )
            body.extend(_comment_lines(proposal.title))
            # `note` before the statement, not after: this script's whole purpose is to be
            # read top-to-bottom before anything is run, and `rationale` never reaches this
            # file at all. See `PostgresWorkloadAdapter.render_ddl` for the same discipline.
            if proposal.note:
                body.extend(_comment_lines(proposal.note))
