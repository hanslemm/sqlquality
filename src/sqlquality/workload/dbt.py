...
def _prepend_note(existing: str | None, dbt_note: str) -> str:
    """`dbt_note` after whatever `existing` note the proposal already carried, not in place
    of it.

    Both the drop-warning and the generic "not expressed as config" paths used to build
    `note` as `_dbt_drop_note(model)` / `_dbt_ddl_note(model, reason)` alone — replacing
    `proposal.note` outright rather than appending to it. That was silent and harmless for
    every rule this module was written against, because no Postgres rule sets `note` before
    enrichment ever sees it (`CREATE INDEX`/`DROP INDEX` proposals arrive with `note=None`).
    It stopped being harmless the moment a rule with a *real*, already-meaningful `note`
    could reach either path: Redshift's ADV105 sets `note` to its own Amazon-Redshift-Advisor
    attribution ("this statement was generated by Redshift itself and is relayed
    verbatim") — exactly the sentence the task that added this module's dbt-interaction
    review required to survive in the rendered DDL file. Overwriting it with only the dbt
    warning would have silently discarded that attribution the instant a dbt-managed
        ...
        if _is_index_dropping(proposal.ddl):
            ...
            return (
                dataclasses.replace(
                    ...
                    rationale=rationale,
                    evidence=evidence,
                    note=_prepend_note(proposal.note, _dbt_drop_note(model)),
                ),
                None,
            )
        # Some other statement for a relation dbt owns — no rule emitted one when this
        # branch was written, and `_is_index_creating` matches by DDL prefix specifically so
